...
from model.assumptions import get_custom_forecast_assumptions, get_historical_average_forecast_assumptions, get_bear_forecast_assumptions, get_bull_forecast_assumptions, get_base_forecast_assumptions
...
def run_model(ticker: str, analysis: str = "standard") -> None:
    ...
    def run_scenario_model(case: str, historical_data: pd.DataFrame, output_dir: Path, outputs_dir: Path) -> None:
        if case == "base":
            ...
            forecast_assumptions = get_bear_forecast_assumptions()
        ...
        outputs_forecast_dir.mkdir(parents=True, exist_ok=True)

        outputs_forecasted_path = outputs_forecast_dir / f"{ticker}_forecasted_data_{case}.csv"
        ...
        print(f"Enterprise Value ({case}): {enterprise_value}")
        print(f"Terminal Value ({case}): {terminal_value}")
        print(f"Equity Value ({case}): {equity_value}")
        print(f"Intrinsic Value per Share ({case}): {intrinsic_value_per_share}")

        # Ensure valuation summaries directory exists
        valuation_summary_dir = outputs_dir / "valuation_summaries"
        valuation_summary_dir.mkdir(parents=True, exist_ok=True)

        summary_path = valuation_summary_dir / f"{ticker}_valuation_summary_{case}.txt"
    ...
    for case in cases:
        run_scenario_model(case, historical_data, output_dir, outputs_dir)
